#!/usr/bin/env python3
"""Build the v7 lead scoring intro CSVs (generates the bundle internally).

Usage:
    python scripts/build_v7_snapshot.py OUTPUT_DIR

Produces two CSV files in OUTPUT_DIR:
- lead_scoring_intro_v7.csv          (student-safe, no leakage columns)
- lead_scoring_intro_v7_instructor.csv (same rows + __leakage__ trap column)

Both are 1000-row files at ~30% conversion rate with:
- Day-20 windowed features
- Structured missingness (MAR + structural + MCAR)
- Purely causal leakage trap: post-snapshot touches from sim events (NO label boost)
...
def build_v7_datasets(seed: int = SEED) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Compute post-snapshot touches from event timeline (purely causal, no boost)
    ...
    # NO boost_leakage_trap() -- v7 uses purely causal trap
...
def main() -> None:
    ...
    df_student, df_instructor = build_v7_datasets()

    student_path = output_dir / "lead_scoring_intro_v7.csv"
    instructor_path = output_dir / "lead_scoring_intro_v7_instructor.csv"
