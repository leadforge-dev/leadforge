...
| `lead_created_at` | string (ISO-8601) | simulation clock | Lead birthday; useful for cohort/time-shift evaluation (see [acceptance gates](/docs/dataset/acceptance-gates) G6.4). Drop or bin it for production models — feeding raw timestamps to a linear model is rarely what you want. |
...
**Caveat.** Per [channel signal audit](https://github.com/leadforge-dev/leadforge/blob/main/docs/release/channel_signal_audit.md),
