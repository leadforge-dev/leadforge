...
validated :class:`~leadforge.schemes.lead_scoring.structure.graph.WorldGraph`.
"""
...
from leadforge.schemes.lead_scoring.structure.graph import GraphValidationError, WorldGraph
...
def sample_hidden_graph(
    ...
    Args:
        ...
        motif_family_name: If provided, pin the motif family by name
            (must be one of
            :data:`~leadforge.schemes.lead_scoring.structure.motifs.MOTIF_FAMILY_NAMES`).
            If ``None``, a family is chosen uniformly at random from the
            five v1 families.

    Returns:
        A validated :class:`~leadforge.schemes.lead_scoring.structure.graph.WorldGraph`.
