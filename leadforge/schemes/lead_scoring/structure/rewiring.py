...
:func:`rewire` takes a :class:`~leadforge.schemes.lead_scoring.structure.motifs.MotifFamily`
and a seeded :class:`~numpy.random.Generator` and returns perturbed lists
of :class:`~leadforge.schemes.lead_scoring.structure.graph.NodeSpec` and
:class:`~leadforge.schemes.lead_scoring.structure.graph.EdgeSpec` that still satisfy the graph
...
from leadforge.schemes.lead_scoring.structure.graph import EdgeSpec, NodeSpec
from leadforge.schemes.lead_scoring.structure.node_types import NodeType

if TYPE_CHECKING:
    import numpy as np

    from leadforge.schemes.lead_scoring.structure.motifs import MotifFamily
...
def rewire(
    ...
    Returns:
        ...
        :class:`~leadforge.schemes.lead_scoring.structure.graph.WorldGraph`.
    """
