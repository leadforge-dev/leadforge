"""Schema contract test for ``bundle_schema_version == "3"``.
...
# Pinned column lists for bundle schema v3.  Update *together* with
# ``BUNDLE_SCHEMA_VERSION`` and ``LEAD_SNAPSHOT_FEATURES``.

V3_TASK_COLUMNS_STUDENT_PUBLIC: frozenset[str] = frozenset(
    {
        "account_id",
        "industry",
        "region",
...
V3_TASK_COLUMNS_RESEARCH_INSTRUCTOR: frozenset[str] = V3_TASK_COLUMNS_STUDENT_PUBLIC | {
    "current_stage",
    "is_sql",
}

V3_LEAD_TABLE_COLUMNS_STUDENT_PUBLIC: frozenset[str] = frozenset(
    {
        "lead_id",
        "contact_id",
        "account_id",
...
V3_LEAD_TABLE_COLUMNS_RESEARCH_INSTRUCTOR: frozenset[str] = V3_LEAD_TABLE_COLUMNS_STUDENT_PUBLIC | {
    "current_stage",
    "is_sql",
}
...
def student_bundle(tmp_path_factory: pytest.TempPathFactory) -> Path:
    out = tmp_path_factory.mktemp("v3_student")
...
def instructor_bundle(tmp_path_factory: pytest.TempPathFactory) -> Path:
    out = tmp_path_factory.mktemp("v3_instructor")
...
def test_manifest_declares_v3(student_bundle: Path, instructor_bundle: Path) -> None:
...
def test_student_public_task_columns_match_v3_contract(student_bundle: Path) -> None:
    actual = _task_cols(student_bundle)
    assert actual == V3_TASK_COLUMNS_STUDENT_PUBLIC, (
        f"student_public task split columns drifted from v3 contract.\n"
        f"  unexpected: {sorted(actual - V3_TASK_COLUMNS_STUDENT_PUBLIC)}\n"
        f"  missing:    {sorted(V3_TASK_COLUMNS_STUDENT_PUBLIC - actual)}\n"
        "  → either update tests/render/test_bundle_schema_v3_contract.py and "
        "bump BUNDLE_SCHEMA_VERSION, or revert the schema change."
    )


def test_research_instructor_task_columns_match_v3_contract(instructor_bundle: Path) -> None:
    actual = _task_cols(instructor_bundle)
    assert actual == V3_TASK_COLUMNS_RESEARCH_INSTRUCTOR, (
        f"research_instructor task split columns drifted from v3 contract.\n"
        f"  unexpected: {sorted(actual - V3_TASK_COLUMNS_RESEARCH_INSTRUCTOR)}\n"
        f"  missing:    {sorted(V3_TASK_COLUMNS_RESEARCH_INSTRUCTOR - actual)}"
    )


def test_student_public_leads_table_columns_match_v3_contract(student_bundle: Path) -> None:
    actual = _leads_cols(student_bundle)
    assert actual == V3_LEAD_TABLE_COLUMNS_STUDENT_PUBLIC, (
        f"student_public tables/leads.parquet columns drifted from v3 contract.\n"
...
def test_research_instructor_leads_table_columns_match_v3_contract(
    ...
    assert actual == V3_LEAD_TABLE_COLUMNS_RESEARCH_INSTRUCTOR, (
        f"research_instructor tables/leads.parquet columns drifted from v3 contract.\n"
        f"  unexpected: {sorted(actual - V3_LEAD_TABLE_COLUMNS_RESEARCH_INSTRUCTOR)}\n"
        f"  missing:    {sorted(V3_LEAD_TABLE_COLUMNS_RESEARCH_INSTRUCTOR - actual)}"
    )
...
def test_is_mql_absent_from_all_v3_artifacts(student_bundle: Path, instructor_bundle: Path) -> None:
    """``is_mql`` was removed entirely in v3 (issue #57): not in the snapshot,
    not in the relational table, not in the feature dictionary, not anywhere."""
    for b in (student_bundle, instructor_bundle):
