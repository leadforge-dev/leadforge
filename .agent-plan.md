...
- [x] `leadforge/structure/sampler.py` — `sample_hidden_graph()` accepts `RNGRoot` instead of bare `seed: int`; deprecated `seed` parameter with backward-compat warnings
- [x] `leadforge/api/generator.py` — passes `RNGRoot(config.seed)` to `sample_hidden_graph()`
...
- [x] 2 new deprecation warning tests in `test_sampler.py`
- [x] All 740 tests pass; lint + format clean
